...
| 개발 기간 | (1차) 2025.06.24 ~ 2025.11.15 </br> (2차) 2026.08.20 ~ 현재 |
| 개발 인원 | (1차) 5인 </br> (2차) 2인 - 이솜귤, 안은혜 |
...
<table>
  ...
  <tbody>
    <tr>
      <td nowrap>이솜귤</br>(팀장)</td>
      <td>학사 데이터 수집 및 저장, 질문 분류 시스템 구축, 벡터DB 기반 질의응답 처리, 관계형DB 기반 졸업사정 로직, 수업 후기 조회 시스템, 대화 맥락 기억, 배포 환경 구성</td>
    </tr>
    <tr>
      <td nowrap>안은혜</td>
      <td>식도락(맛집 추천) 기능 설계·구현, 식도락 관련 RAG/벡터 검색 및 프론트엔드 연결, <br>화면설계서/플로우차트 작성, React 기반 레이아웃 및 CSS 퍼블리싱, 컴포넌트 UI 상태관리, 배포 환경 구성, 전반적인 Git 브랜치 관리</td>
    </tr>
    <tr>
      <td nowrap>김의윤</td>
      <td>fetch+DOMParser 공지사항 기반 크롤링 로직 구현, 자동완성 시스템 구현, FAQ 시스템 설계, JSON 기반 FAQ(클릭형 챗봇) 데이터 구성</td>
    </tr>
    <tr>
      <td nowrap>최민서</td>
      <td>React 컴포넌트 기반 설계, TypeScript Props 인터페이스 정의, 재사용 가능한 모듈 구조, React 상태 관리</td>
    </tr>
    <tr>
      <td nowrap>이여원</td>
      <td>프로젝트 전반 내용 정리 및 결과보고서 작성, 팀 운영 보조</td>
    </tr>
  </tbody>
</table>
